...
The native loop maps keyboard and first-controller XInput state to a raw first-channel GameCube-style PAD sample and applies a DOL-backed subset of stick/trigger filtering, synthesized direction bits, and button transition/repeat state. The controls themselves are a PC policy; scene-dependent repeat updates, remaining PAD behavior, gameplay, and collision remain untranslated. `FUN_8012f3e0` was checked against the verified DOL and must not be described as PAD initialization.
...
Input status: [the DOL-backed PAD-to-player trace](docs/research/input-player-trace.md) identifies polling, a translated first-channel HSD filtering subset, world-map player ownership, and state dispatch. Scene-dependent repeat updates, remaining HSD status handling, action mapping, world-space movement, and collision remain untranslated.
